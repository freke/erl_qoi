...
-define(LUMA_OP(Rd, Gd, Bd),
        ?IN_RANGE_6(Gd) and ?IN_RANGE_4(Rd - Gd) and ?IN_RANGE_4(Bd - Gd)).
...
-spec encode(QOI :: qoi()) -> {ok, <<_:64, _:_*8>>}.
encode(#qoi{width = Width,
            height = Height,
            format = Format,
            color_space = ColorSpace,
            pixels = Pixels}) ->
...
-spec decode(Binary :: <<_:64, _:_*8>>) -> {ok, qoi()}.
...
do_encode(<<Pixel:24, Rest/binary>>,
          rgb = F,
          <<Pixel:24, _/binary>>,
          RunLength,
          Lut,
...
do_decode(<<254:8, R:8, G:8, B:8, Rest/binary>>, Format, Prev, Lut, Acc) ->
  ...
  do_decode(Rest,
            ...
            [maybe_drop_alpha(Pixel, Format) | Acc]);
...
do_decode(<<1:2, Rd:2, Gd:2, Bd:2, Rest/binary>>,
          Format,
          <<Rp:8, Gp:8, Bp:8, A:8>>,
          Lut,
          Acc) ->
...
maybe_drop_alpha(Pixel, rgba) ->
  Pixel;
maybe_drop_alpha(<<R:8, G:8, B:8, _:8>>, rgb) ->
  <<R:8, G:8, B:8>>.
...
diff_luma_color(_, {Rd, Gd, Bd, 0}) when ?LUMA_OP(Rd, Gd, Bd) ->
  ...
  logger:debug("Two byte diff GdLuma:~p RdGdLuma:~p BdGdLuma:~p",
               [GdLuma, RdGdLuma, BdGdLuma]),
...
diff_luma_color_test_() ->
  [?_assertMatch(<<1:2, 3:2, 1:2, 3:2>>,
                 diff_luma_color(<<1, 1, 1, 0>>, wrap_diff(<<1, 1, 1, 0>>, <<0, 2, 0, 0>>))),
   ?_assertMatch(<<2:2, 34:6, 6:4, 7:4>>,
                 diff_luma_color(<<0, 4, 1, 0>>, wrap_diff(<<0, 4, 1, 0>>, <<0, 2, 0, 0>>))),
   ?_assertMatch(<<254, 124, 124, 124>>,
                 diff_luma_color(<<124, 124, 124, 0>>,
                                 wrap_diff(<<124, 124, 124, 0>>, <<0, 2, 0, 0>>))),
   ?_assertMatch(<<254, 0, 0, 6>>,
                 diff_luma_color(<<0, 0, 6, 0>>, wrap_diff(<<0, 0, 6, 0>>, <<0, 2, 0, 0>>))),
   ...
   ?_assertMatch(<<1:2, 3:2, 3:2, 3:2>>,
                 diff_luma_color(<<0, 0, 0, 0>>, wrap_diff(<<0, 0, 0, 0>>, <<255, 255, 255, 0>>))),
   ?_assertMatch(<<1:2, 3:2, 3:2, 3:2>>,
                 diff_luma_color(<<0, 0, 0>>, wrap_diff(<<0, 0, 0>>, <<255, 255, 255>>))),
   ?_assertMatch(<<1:2, 1:2, 1:2, 1:2>>,
                 diff_luma_color(<<255, 255, 255, 0>>,
                                 wrap_diff(<<255, 255, 255, 0>>, <<0, 0, 0, 0>>))),
   ?_assertMatch(<<1:2, 1:2, 1:2, 1:2>>,
                 diff_luma_color(<<255, 255, 255>>, wrap_diff(<<255, 255, 255>>, <<0, 0, 0>>)))].

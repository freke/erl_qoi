...
-spec new(pos_integer(), pos_integer(), 3|4, linear_alpha|all_linear, binary()) -> qoi().
new(Width, Height, Channels, ColorSpace, Pixels) ->
  #{w=>Width, h=>Height, c=>Channels, cs=>ColorSpace, pixels=>Pixels}.

-spec encode(qoi()) -> {ok, binary()}.
encode(#{w:=Width, h:=Height, c:=Channels, cs:=CSpace, pixels:= Pixels}) ->
  Chunks = iolist_to_binary(encode_pixels(Pixels, Channels)),
  C = case Channels of rgb -> 3; rgba -> 4 end,
  {ok, <<"qoif", Width:32, Height:32, C:8, CSpace:8, Chunks/binary, 0:(7*8),1:8>>}.

-spec decode(binary()) -> {ok, qoi()}.
decode(<<"qoif", Width:32, Height:32, Channels:8, CSpace:8, Pixels/binary>>) ->
  C = case Channels of 3 -> rgb; 4 -> rgba end,
  {ok
  , #{w => Width
    , h => Height
    , c => C
    , cs => CSpace
    , pixels => iolist_to_binary(decode_chunks(Pixels, C))
    }
  }.

encode_pixels(Pixels, Channels) ->
  Prev = <<0, 0, 0, 255>>,
  RunLength = 0,
  Lut = #{},
  Acc = [],
  lists:reverse(do_encode(Pixels, Channels, Prev, RunLength, Lut, Acc)).

do_encode(Pixels, Channels, Prev, RunLength, Lut, Acc) when RunLength >= 62 ->
  logger:debug("RunLength reset"),
  RL = bias_run(RunLength),
  do_encode(Pixels, Channels, Prev, 0, Lut, [<<3:2, RL:6>> |Acc]);

do_encode(<<Pixel:32, Rest/binary>>, rgba = C, <<Pixel:32>>, RunLength, Lut, Acc) ->
  logger:debug("Repet RGBA"),
  do_encode(Rest, C, <<Pixel:32>>, RunLength + 1, Lut, Acc);

do_encode(<<Pixel:24, Rest/binary>>, rgb = C, <<Pixel:24>>, RunLength, Lut, Acc) ->
  logger:debug("Repet RGB"),
  do_encode(Rest, C, <<Pixel:24>>, RunLength + 1, Lut, Acc);

do_encode(<<Pixel:32, Rest/binary>>, rgba = C, Prev, 0, Lut, Acc) ->
  logger:debug("New RGBA"),
  {Chunk, NewLut} = handle_non_running_pixel(<<Pixel:32>>, Prev, Lut),
  do_encode(Rest, C, <<Pixel:32>>, 0, NewLut, [Chunk | Acc]);

do_encode(<<RGB:24, Rest/binary>>, rgb = C, Prev, 0, Lut, Acc) ->
  logger:debug("New RGB"),
  Pixel = <<RGB:24, 255:8>>,
  {Chunk, NewLut} = handle_non_running_pixel(Pixel, Prev, Lut),
  do_encode(Rest, C, Pixel, 0, NewLut, [Chunk | Acc]);

do_encode(Pixels, Channels, Prev, RunLength, Lut, Acc) when RunLength > 0 ->
  logger:debug("End of run"),
  RL = bias_run(RunLength),
  do_encode(Pixels, Channels, Prev, 0, Lut, [<<3:2, RL:6>> | Acc]);

do_encode(<<>>, _Channels, _Prev, 0, _Lut, Acc) ->
  logger:debug("Done no current run"),
  Acc;

do_encode(<<>>, _Channels, _Prev, RunLength, _Lut, Acc) ->
  logger:debug("Done end current run"),
  RL = bias_run(RunLength),
  [<<3:2, RL:6>> | Acc].


decode_chunks(Pixels, Channels) ->
  Prev = <<0, 0, 0, 255>>,
  Lut = #{},
  Acc = [],
  lists:reverse(do_decode(Pixels, Channels, Prev, Lut, Acc)).

do_decode(<<0:(7*8),1:8>>, _Channels, _Prev, _Lut, Acc) ->
  logger:debug("End marker"),
  Acc;

do_decode(<<254:8, R:8, G:8, B:8, Rest/binary>>, Channels, Prev, Lut, Acc) ->
  logger:debug("rgb"),
  <<_:24, A:8>> = Prev,
  Pixel = <<R, G, B, A>>,
  do_decode(Rest, Channels, Pixel, update_lut(Lut, Pixel), [maybe_drop_alpha(Pixel, Channels) | Acc]);

do_decode(<<255:8, R:8, G:8, B:8, A:8, Rest/binary>>, Channels, _Prev, Lut, Acc) ->
  logger:debug("rgba"),
  Pixel = <<R, G, B, A>>,
  do_decode(Rest, Channels, Pixel, update_lut(Lut, Pixel), [maybe_drop_alpha(Pixel, Channels) | Acc]);

do_decode(<<0:2, Index:6, Rest/binary>>, Channels, _Prev, Lut, Acc) ->
  logger:debug("Index ~p", [Index]),
  Pixel = maps:get(Index, Lut, <<0:32>>),
  do_decode(Rest, Channels, Pixel, Lut, [maybe_drop_alpha(Pixel, Channels) | Acc]);

do_decode(<<1:2, Rd:2, Gd:2, Bd:2, Rest/binary>>, Channels, <<Rp:8, Gp:8, Bp:8, A:8>>, Lut, Acc) ->
...
do_decode(<<2:2, B_Gd:6, Rd_Gd:4, Bd_Gd:4, Rest/binary>>, Channels, <<Rp:8, Gp:8, Bp:8, A:8>>, Lut, Acc) ->
...
do_decode(<<3:2, Count:6, Rest/binary>>, Channels, Prev, Lut, Acc) ->
  logger:debug("Run"),
  Pixels = binary:copy(maybe_drop_alpha(Prev, Channels), unbias_run(Count)),
  do_decode(Rest, Channels, Prev, Lut, [Pixels | Acc]).
